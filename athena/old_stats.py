...
class Sample:
    ...
    def calculate_exon_stats(self, coverage_data, thresholds) -> pd.DataFrame:
        ...
        exon_coverage = self._calculate_exon_thresholds(
            data=coverage_data, output=exon_coverage, thresholds=thresholds
        )

        return exon_coverage

    def calculate_exon_stats_parallel(
        self, coverage_data, thresholds
    ) -> pd.DataFrame:
        """
        ...
        """
        print(
            f"Calculating exon stats using {multiprocessing.cpu_count()} cores"
        )
        start = time()
        ...
        transcript_array = np.array_split(
            np.array(transcripts), multiprocessing.cpu_count()
        )
        split_dfs = np.asanyarray(
            [
                coverage_data[coverage_data["transcript"].isin(x)]
                for x in transcript_array
            ],
            dtype=object,
        )
    ...
    def calculate_gene_stats(
        self, coverage_data, exon_data, thresholds
    ) -> pd.DataFrame:
        """
        ...
        gene_coverage = self._generate_empty_df(
            per_base_data=exon_data, columns=["gene", "transcript"]
        )
        ...
        gene_coverage = self._calculate_mean(
            data=coverage_data, output=gene_coverage, index=["transcript"]
        )
        ...
        gene_coverage = self._calculate_gene_thresholds(
            data=exon_data, output=gene_coverage, thresholds=thresholds
        )
    ...
    def _calculate_exon_thresholds(
        self, data, output, thresholds
    ) -> pd.DataFrame:
        """
        ...
        for threshold in thresholds:
            ...
            over = data.groupby(["transcript", "exon"]).agg(
                **{
                    ...
                    "exon_length": pd.NamedAgg(
                        column="exon_length", aggfunc=max
                    ),
                }
            )
            ...
            over[f"{threshold}x"] = (
                over[f"{threshold}x"] / over["exon_length"] * 100
            )
    ...
    def _calculate_gene_thresholds(
        self, data, output, thresholds
    ) -> pd.DataFrame:
        """
        ...
        gene_length = data.groupby(["transcript"], as_index=False)[
            "exon_length"
        ].sum()
        gene_length.rename(
            columns={"exon_length": "gene_length"}, inplace=True
        )
        ...
        summed_thresholds = merged.groupby(["transcript"]).agg(
            {x: "sum" for x in thresholds}
        )

        summed_thresholds = summed_thresholds.apply(
            lambda x: round(x, ndigits=12)
        )

        data.drop(["exon_length"], axis=1, inplace=True)

        return pd.merge(
            output, summed_thresholds, on="transcript", validate="1:1"
        )
...
class Multi:
    ...
    def combine_per_base_coverage(
        ...
        for idx, (per_base_coverage, hsmetrics) in enumerate(sample_data):
            ...
            sample_bases[idx] = int(hsmetrics["ON_TARGET_BASES"]) * float(
                hsmetrics["PCT_USABLE_BASES_ON_TARGET"]
            )
            ...
            per_base_coverage.rename(
                {"cov": f"S{idx}_raw", "cov_norm": f"S{idx}_norm"},
                axis=1,
                inplace=True,
            )
    ...
    def calculate_exon_stats(self, all_exon_stats) -> pd.DataFrame:
        ...
        for sample in all_exon_stats:
            run_exon_stats.append(
                self._normalise_sample(
                    exon_stats=sample[0],
                    hsmetrics=sample[1],
        ...
        for column in calculate_columns:
            column_stats = (
                ...
                .agg(
                    **{
                        column: pd.NamedAgg(column=column, aggfunc=sum),
                        f"{column}_std": pd.NamedAgg(
                            column=column, aggfunc=np.std
                        ),
                    }
                )
                .reset_index()
            ...
            output = pd.merge(
                output,
                column_stats[["transcript", "exon", column, f"{column}_std"]],
                on=["transcript", "exon"],
                validate="1:1",
            )
    ...
    def calculate_gene_stats(self, run_exon_stats) -> pd.DataFrame:
        ...
        for column in calculate_columns:
            column_stats = (
                run_exon_stats.groupby(["gene", "transcript"], observed=True)
                .agg(
                    **{
                        column: pd.NamedAgg(column=column, aggfunc=np.mean),
                        f"{column}_std": pd.NamedAgg(
                            column=column, aggfunc=np.std
                        ),
                    }
            ...
            output = pd.merge(
                output,
                column_stats[["transcript", column, f"{column}_std"]],
                on=["transcript"],
                validate="1:1",
            )
